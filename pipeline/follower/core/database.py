"""
Database layer for Phase 3 follower signal system.

Handles:
1. Read-only access to Phase 2 analyzed_traders.db
2. Storage of follower signals and position snapshots
"""
...
class Phase2Reader:
    ...
    Queries good traders based on score threshold.
    """
    ...
    async def get_good_traders(
        self,
        score_threshold: int = 90,
        limit: Optional[int] = None
    ) -> List[Dict]:
        """
        Get traders with exceptionally good performance.

        Args:
            score_threshold: Minimum score to include (default: 90)
    ...
    async def get_trader_count(self, score_threshold: int = 90) -> int:
        """
        Get count of good traders matching threshold.

        Args:
            score_threshold: Minimum score to include

        Returns:
            Count of good traders
        """
        query = """
...
class FollowerDatabase:
    """
    Manages the Phase 3 database for follower signals.
    ...
    def __init__(self, db_path: str, connection_timeout: int = 30):
        ...
        Args:
            db_path: Path to follower_signals.db
    ...
    async def initialize(self):
        ...
        async with self._get_connection() as db:
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS follower_signals (
                    ...
                    -- Count-based metrics
                    good_traders_total INTEGER NOT NULL,
        ...
        logger.info(f"Follower database initialized at {self.db_path}")
    ...
    async def save_signal(self, signal: Dict):
        """
        Save a follower signal to the database.
        ...
        async with self._get_connection() as db:
            await db.execute("""
                INSERT INTO follower_signals (
                    ...
                    good_traders_total, long_count, short_count,
    ...
    async def get_signal_statistics(self, coin: Optional[str] = None) -> Dict:
        ...
        async with self._get_connection() as db:
            # Total signals
            query = f"SELECT COUNT(*) FROM follower_signals {where_clause}"
            async with db.execute(query, params) as cursor:
