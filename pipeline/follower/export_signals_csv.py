#!/usr/bin/env python3
"""
CSV Export Tool for Follower Signals Database

Exports signals from follower_signals.db to CSV format.
Can export both follower signals and position snapshots.
"""
...
def export_signals_to_csv(db_path: str, output_path: str, limit: int = None, coin: str = None):
    """
    Export follower signals to CSV.
    ...
    # Build query
    query = "SELECT * FROM follower_signals"
    params = []
...
def show_database_stats(db_path: str):
    ...
    # Signals count
    cursor.execute("SELECT COUNT(*) FROM follower_signals")
...
Examples:
  ...
  python export_signals_csv.py --output my_signals.csv
    ...
    if args.output:
        ...
        filename = f"follower_{args.type}{coin_suffix}{limit_suffix}_{timestamp}.csv"
