"""
Follower signal generator.

Generates trading signals by following good traders' positioning.
...
class FollowerSignalGenerator:
    """
    Generates follower trading signals from aggregated position data.

    Logic: If good traders are heavily positioned one way, signal the same direction.
    """
    ...
    def generate_signals(
        ...
        """
        Generate follower signals for all coins.
    ...
    def generate_signal_for_coin(
        ...
        """
        Generate follower signal for a single coin.
        ...
        # Generate follower signal based on good traders' positioning
        ...
        signal = {
            ...
            'good_traders_total': total_traders,
    ...
    def _calculate_signal(
        ...
        Follower logic:
        - If good traders are mostly LONG → Signal LONG
        - If good traders are mostly SHORT → Signal SHORT

        Args:
            long_pct: Percentage of good traders long (0-100)
            short_pct: Percentage of good traders short (0-100)
        ...
        # Check LONG positioning (generate LONG signal)
        if long_pct >= strong_threshold:
            return 'LONG', 'STRONG'
        elif long_pct >= moderate_threshold:
            return 'LONG', 'MODERATE'

        # Check SHORT positioning (generate SHORT signal)
    ...
    def explain_signal(self, signal: Dict) -> str:
        ...
        # Determine what good traders are doing
        if direction == 'LONG':
            good_trader_bias = f"{long_pct:.1f}% of good traders are LONG"
        else:
            good_trader_bias = f"{short_pct:.1f}% of good traders are SHORT"

        explanation = (
            f"{coin}: {strength} {direction} signal "
            f"(confidence: {confidence:.2f})\n"
            f"  → {good_trader_bias} {metric_name}\n"
            f"  → Follower strategy suggests: {direction} {coin}"
        )
