"""
Rich-based console dashboard for follower signals.

Beautiful, real-time terminal UI showing good trader positioning and follower signals.
"""
...
class FollowerDashboard:
    """
    Live-updating console dashboard for follower signals.
    ...
    def render(
        ...
        Args:
            signals: List of signal dictionaries
            good_traders_count: Total number of good traders analyzed
    ...
    def _create_header(
        ...
        header_text = Text()
        header_text.append("FOLLOWER SIGNALS - Good Traders Analysis\n", style="bold cyan")
        header_text.append(f"Updated: {timestamp}\n\n", style="dim")
        header_text.append(f"Total Good Traders: {good_traders_count}  |  ", style="white")
    ...
    def _create_signal_table(self, signals: List[Dict]) -> Table:
        ...
        """
        table = Table(
            title="Follower Trading Signals",
        ...
        for signal in signals:
            ...
            total_traders = signal['good_traders_total']
    ...
    def _create_position_bar(
        ...
        # Color codes - for follower strategy
        # Green when good traders are heavily LONG (we follow them)
        # Red when good traders are heavily SHORT (we follow them)
        if long_pct > 70:
            bar = f"[green]{'█' * long_bars}[/green][dim]{'░' * short_bars}[/dim]"
        elif short_pct > 70:
            bar = f"[dim]{'░' * long_bars}[/dim][red]{'█' * short_bars}[/red]"
        else:
    ...
    def print_static(
        ...
        Args:
            signals: List of signal dictionaries
            good_traders_count: Total good traders
    ...
    def print_summary(self, signals: List[Dict]):
        ...
        for signal in actionable[:10]:  # Top 10
            ...
            if direction == 'SHORT':
                reason = f"{short_pct:.0f}% of good traders are SHORT"
                style = "red"
            else:
                reason = f"{long_pct:.0f}% of good traders are LONG"
                style = "green"
